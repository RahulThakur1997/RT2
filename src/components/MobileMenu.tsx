...
export const MobileMenu: React.FC<MobileMenuProps> = ({
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-[60] bg-[#151D45]/98 backdrop-blur-lg flex flex-col justify-between overflow-y-auto transition-all duration-300 animate-in fade-in"
    >
      {/* Top Header Row: ONLY logo image on left, close button on right */}
      <div className="flex items-center justify-between px-5 py-4 border-b border-white/15 sticky top-0 bg-[#151D45]/95 backdrop-blur-md z-10">
        <div onClick={() => handleLinkClick('hero')} className="cursor-pointer" title="Keystone Roofing & Building Ltd">
        ...
        <button
          ...
          className="min-h-[44px] min-w-[44px] flex items-center justify-center rounded-lg bg-white/10 hover:bg-[#AC0E13] text-white transition-colors cursor-pointer"
        >
          <X className="w-6 h-6" />
        </button>
      </div>
      ...
      <div className="flex-1 px-5 py-6 space-y-6">
        ...
        <div>
          <span className="text-[11px] font-heading font-bold uppercase tracking-widest text-blue-200/70 px-2 block mb-3">
            Navigation
          </span>
          <div className="space-y-2">
            {MAIN_NAV_ITEMS.map((item) => (
              <button
                ...
                className="w-full min-h-[52px] px-4 py-3 rounded-xl bg-white/[0.05] hover:bg-white/[0.12] border border-white/10 text-left font-heading font-bold text-[17px] text-white hover:text-[#AC0E13] transition-colors flex items-center justify-between group cursor-pointer"
              >
                <span>{item.label}</span>
                <span className="text-white/40 group-hover:text-[#AC0E13] group-hover:translate-x-1 transition-all font-sans text-lg">
                  →
                </span>
              </button>
            ))}
        ...
        <div className="p-4 rounded-xl bg-gradient-to-br from-[#242F6B] to-[#151D45] border border-white/15 shadow-lg flex flex-col gap-3">
          <button
            ...
            className="min-h-[50px] w-full rounded-lg bg-[#AC0E13] hover:bg-[#920b10] text-white font-heading font-bold text-base tracking-wider uppercase shadow-md flex items-center justify-center gap-2 transition-transform active:scale-[0.98] cursor-pointer"
          >
        ...
        <div className="pt-2 border-t border-white/15 space-y-2.5 text-xs text-gray-300">
          <div className="flex items-start gap-2">
            <MapPin className="w-4 h-4 text-[#AC0E13] shrink-0 mt-0.5" />
          ...
          <div className="flex items-center gap-2">
            <Mail className="w-4 h-4 text-[#AC0E13] shrink-0" />
            <a
          ...
          <div className="flex items-center justify-between pt-1">
            ...
            <a
              ...
              aria-label="Keystone Roofing Instagram Profile"
              className="min-h-[44px] min-w-[44px] flex items-center justify-center rounded-lg bg-white/10 text-white hover:text-[#AC0E13] transition-colors"
            >
